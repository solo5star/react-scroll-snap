...
const ScrollSnapImplItemList = () => {
  const { scrollPosition, enableRolling, items, itemRenderer } = useScrollSnapContext();
    ...
    >
      {([0, 1, -1] as const)
        ...
        .map(({ item, index, visibleIndex }) => (
          <ScrollSnapItemProvider key={index} offset={visibleIndex} position={visiblePosition}>
            <div
              style={{
                height: '100%',
                gridArea: '1 / 1 / 1 / 1',
            ...
            >
              {typeof index === 'number' && itemRenderer(item, index)}
            </div>
